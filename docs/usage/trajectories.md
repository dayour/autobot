...
The [`trajectories/`](https://github.com/autobot/autobot/tree/main/trajectories) folder is the default location that experiment results (for example invocations of [`autobot run`](cl_tutorial.md)) will be written to.

## Trajectories

The main output file is `<instance_id>.traj`, which is a `.json` formatted file containing the (thought, action, observation) turns generated by autobot towards solving `<instance_id>`.
...
!!! warning "Query and message field"
    Prior to autobot 1.1.0, we had a `message` field which corresponded (approximately) to the input
...
Note: This trajectory is from autobot v0.7.0. It might look different in newer versions.
...
Trajectories can also be turned into custom demonstrations for autobot ([more information](../config/demonstrations.md)).

## Other output files

Together with the trajectory, autobot will also generate the following files per instance:

* `config.yaml`: A yaml file containing the configuration used for the experiment. You can directly repeat the run of this
  exact instance by running `autobot run --config config.yaml`.
...
Here are a few more files that are generated with `autobot run-batch`:
...
* `run_batch.config.yaml`: A yaml file containing the configuration used for the experiment. You can directly repeat the run of this
  exact experiment by running `autobot run-batch --config run_batch.config.yaml`.
* `preds.json`: All predictions (i.e., patches produced by the agent) from all instances of the experiment.
  If this file goes out of date, you can regenerate it by running `autobot merge-preds`.
* `run_batch.*.log`: Log files at different levels.
* `run_batch_exit_statuses.yaml`: A yaml file containing the exit statuses of all instances in the experiment (for this run of `autobot run-batch`).


!!! tip
    * Evaluation is not completed by `autobot run-batch`, it is a separate step (see [batch mode](batch_mode.md))
    * `preds.json` can be referenced directly into autobot-bench to run evaluation
    * Trajectories can be turned into custom demonstrations for autobot ([more information](../config/demonstrations.md)).
