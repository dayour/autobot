# autobot 1.0.0 migration guide

Welcome to autobot 1.0! So much new stuff! Here's a quick rundown of the cool new things you can do:

* :sparkles: Fast, massively parallel code execution with [SWE-ReX](https://github.com/autobot/SWE-ReX).
* :sparkles: Run autobot locally but execute code in the cloud (using modal, AWS, or anything else that runs [SWE-ReX](https://github.com/autobot/SWE-ReX)).
...
If you're familiar with the old autobot, here are the main changes you need to be aware of.

!!! tip "autobot 0.7"
    The old autobot before the major overhaul is still preserved on the `v0.7` branch.
    In particular, this is still the version to use for the autobot EnIGMA configuration.
...
* Instead of the `run.py` script, we now have a central `autobot` entry point that you can call after installing the package.
  This should be much more convenient!
* Instead of one `run.py` command, we now have several subcommands: `autobot run` to run over single issues, `autobot run-batch` to run over a whole batch of issues, and various utility commands. Run `autobot --help` to see all options. Splitting up both commands made it easier to make both use cases more convenient and flexible.
...
* The biggest change is [SWE-ReX](https://github.com/autobot/SWE-ReX), our new "backend" that handles all code execution.
* As a result of this, the `autobotenv` class is basically gone and only is a small wrapper around a `swerex` runtime
