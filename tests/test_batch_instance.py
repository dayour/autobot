...
from autobot.agent.problem_statement import TextProblemStatement
from autobot.environment.repo import PreExistingRepoConfig
from autobot.run.batch_instances import BatchInstance, SimpleBatchInstance, autobotbenchInstances, _slice_spec_to_slice


def test_simple_batch_from_autobot_bench_to_full_batch_instance(test_data_sources_path):
    sb_instance = json.loads((test_data_sources_path / "autobot-bench-dev-easy.json").read_text())[0]
    instance = SimpleBatchInstance.from_autobot_bench(sb_instance).to_full_batch_instance(
    ...
    assert instance.env.deployment.image == "docker.io/autobotbench/sweb.eval.x86_64.pydicom_1776_pydicom-1458:latest"
...
@pytest.mark.slow
def test_get_autobot_bench_instances():
    for subset in ["lite", "verified", "full"]:
        for split in ["dev", "test"]:
            if subset in ["verified", "multilingual"] and split == "dev":
                continue
            print(subset, split)
            instance_config = autobotbenchInstances(subset=subset, split=split)  # type: ignore
