...
from autobot.run.hooks.open_pr import _remove_triple_backticks, format_trajectory_markdown
...
def test_is_github_repo_url():
    assert _is_github_repo_url("https://github.com/autobot/autobot")
    assert _is_github_repo_url("https://github.com/autobot/autobot/anything")
    assert _is_github_repo_url("github.com/autobot/autobot/anything")
    assert not _is_github_repo_url("")
    assert not _is_github_repo_url("/path/to/file")


def test_parse_gh_repo_url():
    assert _parse_gh_repo_url("https://github.com/autobot/autobot") == ("autobot", "autobot")
    assert _parse_gh_repo_url("github.com/autobot/autobot") == ("autobot", "autobot")
    assert _parse_gh_repo_url("github.com/autobot/autobot/asdfjsdfg") == ("autobot", "autobot")
    assert _parse_gh_repo_url("git@github.com/autobot/autobot/asdfjsdfg") == ("autobot", "autobot")
...
def test_parse_gh_issue_url():
    url = "https://github.com/autobot/autobot/issues/43"
    owner, repo, no = _parse_gh_issue_url(url)
    assert owner == "autobot"
    assert repo == "autobot"
    assert no == "43"
...
def test_is_from_github_url():
    ...
    assert _is_github_issue_url("https://github.com/autobot/autobot/issues/43")
