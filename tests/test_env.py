...
from autobot.environment.hooks.abstract import EnvHook

from .conftest import autobot_env_context


@pytest.mark.slow
def test_init_autobot_env(test_env_args):
    with autobot_env_context(test_env_args):
        pass


@pytest.mark.slow
def test_init_autobot_env_conservative_clone(test_env_args):
    with mock.patch.dict("os.environ", {"autobot_CLONE_METHOD": "full"}):
        with autobot_env_context(test_env_args):
            pass
...
def test_read_file(tmp_path, test_env_args):
    with autobot_env_context(test_env_args) as env:
        content = env.read_file(Path("tests/filetoread.txt"))
        assert content.splitlines()[-1].strip() == "autobotenv.read_file"
